...
export function SessionSelector() {
    ...
    const handleFetch = useCallback(async (search_param) => {
        await getSessions(search_param).then((values) => {
            setSessions(values);
        });
    }, []);
    ...
    const handleAddSessions = async (sessions) => {
        sessions = sessions.map((session_date) => ({
            session_date: format(session_date, 'yyyy-MM-dd', { local: { code: 'fr-FR' } }),
        }));
        const { data, error } = await addSession({ sessions }).then(async (values) => {
            await handleFetch();
            return values;
        });
        if (error) {
    ...
    const handleDeleteSession = async (session_id) => {
        await deleteSession({ session_id }).then(
            async (values) => {
                await handleFetch();
                console.info(values);
                return values;
        ...
        >
            <Flex direction="column" gap="4" width="max-content">
                ...
                <Button
                    ...
                    onClick={() => handleAddSessions(selectedDates)}
                >
                    Créer le(s) session(s)
                </Button>
            </Flex>
            <Separator orientation="vertical" size="4" />
            <Flex direction="column" gap="5">
                <Heading size="3" style={{ margin: '.5rem 0' }}>
                    Sessions à venir
                </Heading>
                <TextField.Root style={{ maxWidth: 240, padding: '0 2.5rem' }}>
                    <TextField.Input
                        ...
                        value={searchSession}
                        onChange={(e) =>
                            e ? setSearchSession(e.target.value) : setSearchSession('')
                        }
                    />
                </TextField.Root>
                ...
                <ScrollArea
                    type="auto"
                    scrollbars="vertical"
                    style={{ height: '300px', padding: '0 15px 0 0' }}
                >
                                        ...
                                        <Card key={key} style={{ maxWidth: 240 }}>
                                            <Flex gap="3" justify="center" align="center">
                                                ...
                                                <IconButton
                                                    ...
                                                    color="crimson"
                                                    size="1"
                                                    onClick={() => handleDeleteSession(el.id)}
                                                >
                                                    <TrashIcon />
                                                </IconButton>
                                            </Flex>
